...
from woob.browser.selenium import (IsHereCondition, SeleniumBrowser,
                                   SeleniumPage, SubSeleniumMixin, webdriver)
...
class CenterBookingPage(JsonPage):
    ...
    def get_agenda_ids(self, motive_id, practice_id=None):
        ...
        for a in self.doc["data"]["agendas"]:
            if (
                motive_id in a["visit_motive_ids"]
                and not a["booking_disabled"]
                and (not practice_id or a["practice_id"] == practice_id)
            ):
...
class AvailabilitiesPage(JsonPage):
    def find_best_slot(self, time_window=1):
        for a in self.doc["availabilities"]:
            if time_window and parse_date(
                a["date"]
            ).date() > datetime.date.today() + relativedelta(days=time_window):
                continue
...
class AppointmentsPage(HTMLPage):
    pass
...
class DoctolibLoginBrowser(SeleniumBrowser):
    BASEURL = "https://www.doctolib.fr"

    login = URL(r"/sessions/new", SeleniumPage)
    two_factor = URL(r"/sessions/two-factor", SeleniumPage)
    appointments = URL(r"/account/appointments", SeleniumPage)
    ...
    def do_login(self, username, password):
        ...
        username_field = driver.find_element_by_xpath(r"//input[@id='username']")
        ...
        login_button = driver.find_element_by_xpath(
            "//div[@data-test='login-card']//button"
        )
        login_button.click()

        self.wait_until(IsHereCondition(self.two_factor), timeout=1000 * 60)

        print("Getting pin.")

        pin = ""

        while len(pin.strip()) != 6:
            pin = input("Enter the pin you have received from doctolib.")
        ...
        for i, digit in enumerate(pin):
            pin_input_field = driver.find_element_by_xpath(
                f"//div[@class='pin-input-group']//input[position()={i+1}]"
            )
            pin_input_field.send_keys(digit)
            actions = ActionChains(driver)
            actions.perform()
            sleep(0.5)

        self.wait_until(IsHereCondition(self.appointments), timeout=1000 * 60 * 10)
...
class Doctolib(SubSeleniumMixin, TwoFactorBrowser):
    BASEURL = "https://www.doctolib.fr"

    SELENIUM_BROWSER = DoctolibLoginBrowser
    ...
    appointments = URL(r"/account/appointments", AppointmentsPage)
    ...
    def do_login(self):
        ...
        try:
            ...
            log("Two factor login, switching to graphical login.")

            self.do_graphical_login()

            return True

        return True

    def do_graphical_login(self):
        ...
        try:
            ...
            sub_browser.do_login(self.username, self.password)
    ...
    def try_to_book(self, center, time_window=1):
        ...
        for place in self.page.get_places():
            ...
            if self.try_to_book_place(
                profile_id, motive_id, practice_id, agenda_ids, time_window
            ):
                return True

        return False

    def try_to_book_place(
        self, profile_id, motive_id, practice_id, agenda_ids, time_window=1
    ):
        ...
        self.second_shot_availabilities.go(
            params={
                "start_date": slot["steps"][1]["start_date"].split("T")[0],
                "visit_motive_ids": motive_id,
                "agenda_ids": "-".join(agenda_ids),
        ...
        log(
            "  ├╴ Second shot: %s", parse_date(second_slot["start_date"]).strftime("%c")
        )
        ...
        self.appointment_post.go(
            id=a_id, data=json.dumps(data), headers=headers, method="PUT"
        )

        if (
            "redirection" in self.page.doc
            and not "confirmed-appointment" in self.page.doc["redirection"]
        ):
            log(
                "  ├╴ Open %s to complete",
                "https://www.doctolib.fr" + self.page.doc["redirection"],
            )
...
class Application:
    ...
    def main(self):
        ...
        parser.add_argument(
            "--pfizer", "-z", action="store_true", help="select only Pfizer vaccine"
        )
        parser.add_argument(
            "--moderna", "-m", action="store_true", help="select only Moderna vaccine"
        )
        parser.add_argument(
            "--patient", "-p", type=int, default=-1, help="give patient ID"
        )
        parser.add_argument(
            "--time-window",
            "-t",
        ...
        if args.patient >= 0 and args.patient < len(patients):
            ...
            while True:
                print(
                    "For which patient do you want to book a slot?", end=" ", flush=True
                )
                try:
        ...
        log(
            "Starting to look for vaccine slots for %s %s in %s next day(s)...",
            docto.patient["first_name"],
            docto.patient["last_name"],
            args.time_window,
        )
        ...
        try:
            ...
            print(
                "\n%s: City %s not found. For now Doctoshotgun works only in France."
